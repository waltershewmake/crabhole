...
pub struct LineState {
    ...
    prompt: String,
    prompt_len: usize,
    pub should_print_line_on_enter: bool, // After pres: strip_str(&prompt).len(),sing enter, should we print the line just submitted?
...
impl LineState {
    pub fn new(prompt: String, term_size: (u16, u16)) -> Self {
        let prompt_len = strip_str(&prompt).len();
        let current_column = prompt_len as u16;
        Self {
            prompt_len,
            prompt,
    ...
    pub fn render(&self, term: &mut impl Write) -> io::Result<()> {
        ...
        let line_len = self.prompt_len + UnicodeWidthStr::width(&self.line[..]);
    ...
    pub fn update_prompt(
        ...
        self.clear(term)?;
        self.prompt_len = strip_str(&prompt).len();
        self.prompt.clear();
